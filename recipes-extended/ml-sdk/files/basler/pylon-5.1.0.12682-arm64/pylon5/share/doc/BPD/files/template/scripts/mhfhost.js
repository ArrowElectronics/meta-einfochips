...
function displayTopics( a_QueryResult )
{
	...
	{	
		/* EDIT: Remove the rhsyn (= synonym) part from search result URLs if empty */
		gstrSyn = gstrSyn.trim();
		if (rh.util.isUsefulString(gstrSyn))
			var strParams = "?" + RHHIGHLIGHTTERM + "=" + encodeURIComponent( szSearchStrings ) + "&" + RHSYNSTR + "=" + encodeURIComponent(gstrSyn);
		else
			var strParams = "?" + RHHIGHLIGHTTERM + "=" + encodeURIComponent( szSearchStrings );
        /* END EDIT */
