...
def run_inference(runtime, net_id, images, labels, input_binding_info, output_binding_info):
    ...
    for idx, im in enumerate(images):
        ...
        print(f"Time to run inference : {int(delta.total_seconds() * 1000)} ms")
        print_top_n(5, results, labels, out_tensor)

def run_inference_on_frame(runtime, net_id, pil_image, labels, input_binding_info, output_tensors):
    """Runs inference on a set of images.
    ...
    start_time = datetime.datetime.now()
    # Create input tensors
    input_tensors = ann.make_input_tensors([input_binding_info], [pil_image])
    ...
    result_txt = get_top_1(results, labels, out_tensor)
...
def parse_command_line(desc: str = ""):
    ...
    parser.add_argument("-c", "--cam",
                        help="Number for device id of the camera", action="store",
                        default=0)
...
def load_image(pil_image, input_width: int, input_height: int, data_type=np.uint8,
                scale: float = 1., mean: list = [0., 0., 0.], stddev: list = [1., 1., 1.],
                preprocess_fn=preprocess_default):
    """Loads images, resizes and performs any additional preprocessing to run inference.
    ...
    img = preprocess_fn(pil_image, input_width, input_height, data_type, scale, mean, stddev)
    return img
...
def get_top_1(results: list, labels: list, prob: list):
    """Prints TOP-N results
    ...
    # result = "class={0} | value={1}".format(labels[results[0]], prob[results[0]])
    result = "class={0}".format(labels[results[0]])
    return result
