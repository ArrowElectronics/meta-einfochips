...
dataset = 'video_training_data_face_rec'
video_filename = 'Now_You_See_Me.mp4'

recognizer_filename = 'recognizer_video_sklearn.pickle'
labels_filename = 'labels_video_sklearn.pickle'
...
def capture_new_training_set(label):
    ...
    cam = FileVideoStream(src=args["videosrc"]).start()
    time.sleep(1.0)
...
def resized_frame(frame):
    # TODO : resize frame for fix window size
    frame = imutils.resize(frame, width=640)
    return frame
...
def recognize_faces():
    ...
    # initialize the video stream, then allow the camera sensor to warm up
    print("Starting video stream...")
    fvs = FileVideoStream(video_filename, transform=resized_frame).start()
    time.sleep(1.0)
